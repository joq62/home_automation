...
-record(state, {max_events,events,event_num}).
%% ====================================================================
%% External functions
%% ====================================================================
...
%% --------------------------------------------------------------------
init([InitArgs]) ->
    Event=lists:append(node(),[{event_level,info},{event_info,[?MODULE,?LINE,'service started',?MODULE]}]),
    Events=log_lib:add_event(Event,?MAX_EVENTS,[]),
       
    {ok, #state{max_events=?MAX_EVENTS,events=Events,event_num=0}}.    
...
handle_cast({add_event,Event},State) ->
    ...
    {node_id,NodeId}=lists:keyfind(node_id,1,Event),
    {module,Module}=lists:keyfind(module,1,Event),
